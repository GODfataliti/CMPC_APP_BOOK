...
export function BookRequested() {
  ...
  const onSearch = async() => {
    setDisabled(true);
    const values = inputSearch.split(' ')
    const search: any = {
      page: 1,
      // PARAMS
      firstName: values[0] ?? '',
  ...
  return (
    <section className="w-full flex flex-col m-2 p-2 gap-2">
      {advancedSearch ? (
        <p className="text-sm text-start font-medium">Busca por título, autor, editorial o disponibilidad del libro</p>
      ) : (
      ...
      <div className="flex flex-row gap-2 justify-between items-center w-full">
        ...
        <div className="flex flex-col gap-1">
          <Button onClick={() => setAdvancedSearch(!advancedSearch)}>Avanzado</Button>
          <Button
            onClick={onSearch}
            variant="default"
