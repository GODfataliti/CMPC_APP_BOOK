...
export async function getBooksByParams(props: QueryParams): Promise<BookResultsData> {
    try {
      ...
      const response: GetBooksRES = await fetch(`${VITE_API_URL}/book/search?${queryParams.toString()}`, options)
        .then((res) => res.json());
